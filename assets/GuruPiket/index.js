$(document).ready(function () {
	...
	$('#submit-piket').click(function (e) {
		e.preventDefault();
		if ($("#form-guru-piket").valid()) {
			if ($('input:checked').length > 0) {
				...
				Fn_Submit_Form($('#form-guru-piket').serialize())
			} else {
				Swal.fire({
					icon: 'warning',
					title: 'Oops...',
					text: 'Harap pilih guru piket !',
	...
	function Fn_Submit_Form(DataForm) {
		$.ajax({
			dataType: "json",
			type: "POST",
			url: $('meta[name="base_url"]').attr('content') + "GuruPiket/Store",
			data: DataForm,
			success: function (response) {
				Swal.close()
				if (response.code == 200) {
					$("#TableData").DataTable().ajax.reload();
					$('#form-guru-piket')[0].reset();
					Swal.fire({
						icon: 'success',
						title: 'Success!',
						text: response.msg,
