$(document).ready(function () {
	...
	function Fn_Initialized_DataTable() {
		$("#TableData").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "OverTime/DT_Monitoring_OverTime",
				dataType: "json",
				type: "POST",
				data: {
					from: $('#from').val(),
	...
	$(document).on('click', '.btn-delete', function () {
		...
		Swal.fire({
			...
			text: `Are you sure to dete over time data : ${nama}, Action cannot be rollback !`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				$.ajax({
					url: $('meta[name="base_url"]').attr('content') + "OverTime/Delete",
					type: "post",
					dataType: "json",
					data: {
						SysId: SysId
	...
	$(document).on('click', '.btn-edit', function () {
		$("#location").empty()
		$.ajax({
			url: $('meta[name="base_url"]').attr('content') + "OverTime/Edit_OverTime",
			type: "GET",
			data: {
