$(document).ready(function () {
	...
	var TableData = $("#TableData").DataTable({
		...
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "Master/DT_List_Class",
			dataType: "json",
			type: "POST",
		},
		columns: [{
			...
			},
			{
				...
				render: function (data, type, row, meta) {
					if (data == 1) {
                        ...
                        <button class="btn btn-sm btn-icon btn-warning btn-edit" data-pk="${row.SysId}" data-toggle="tooltip" title="Change Homeroom Teacher"><i class="fas fa-smile"></i></button>`
					} else {
	...
	function Init_Submit_Form(DataForm) {
		...
		$.ajax({
			dataType: "json",
			type: "POST",
			url: $('meta[name="base_url"]').attr('content') + "Master/Store_New_Class",
	...
	$(document).on('click', '.btn-edit', function () {
		...
		$.ajax({
			type: "GET",
			url: $('meta[name="base_url"]').attr('content') + "Master/Append_modal_update_homeroom_teacher",
			data: {
				SysId: SysId
			},
			beforeSend: function () {
