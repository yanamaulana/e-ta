...
$(document).ready(function () {
	...
	$('#submit-lemburan').click(function (e) {
		e.preventDefault();
		if ($("#form-lemburan").valid()) {
			if ($('input:checked').length > 0) {
				...
				Fn_Submit_Form($('#form-lemburan').serialize())
			} else {
				Swal.fire({
					icon: 'warning',
					title: 'Oops...',
	...
	function Fn_Submit_Form(DataForm) {
		$.ajax({
			dataType: "json",
			type: "POST",
			url: $('meta[name="base_url"]').attr('content') + "OverTime/Store",
			data: DataForm,
			success: function (response) {
				Swal.close()
				if (response.code == 200) {
					$("#TableData").DataTable().ajax.reload();
					$('#form-lemburan')[0].reset();
					Swal.fire({
						icon: 'success',
						title: 'Success!',
						text: response.msg,
