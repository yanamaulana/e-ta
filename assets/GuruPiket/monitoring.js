$(document).ready(function () {
	...
	function Fn_Initialized_DataTable() {
		$("#TableData").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "GuruPiket/DT_Monitoring",
				dataType: "json",
				type: "POST",
				data: {
					from: $('#from').val(),
	...
	$(document).on('click', '.btn-delete', function () {
		...
		Swal.fire({
			...
			text: `Apakah anda yakin untuk menghapus data piket : ${nama}, Action ini tidak dapat dirollback !`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				$.ajax({
					url: $('meta[name="base_url"]').attr('content') + "GuruPiket/Delete",
					type: "post",
					dataType: "json",
					data: {
						SysId: SysId
	...
	$(document).on('click', '.btn-edit', function () {
		$("#location").empty()
		$.ajax({
			url: $('meta[name="base_url"]').attr('content') + "GuruPiket/Edit",
			type: "GET",
			data: {
